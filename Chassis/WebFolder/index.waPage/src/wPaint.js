(function($) {
    ...
    function Paint(el, options) {
        ...
        // attributs nécessaires à la gestion du zoom
    ...
    Paint.prototype = {
        generate: function() {
            ...
            function canvasMousewheel(e) {
                //on récupère le facteur de zoom
                var deltaY = 0;
                e.preventDefault();
            ...
            //            createCanvas('').on('mousedown', canvasMousedown).on('wheel', canvasMousewheel).bindMobileEvents();
            this.canvas.addEventListener('wheel', canvasMousewheel);
            ...
            //            $(document).on('mousemove', documentMousemove).on('mousedown', $.proxy(this._closeSelectBoxes, this)).on('mouseup', documentMouseup).on('wheel', $.proxy(this._closeSelectBoxes, this));
        ...
        resize: function() {
            ...
            this.width = this.$el.width();
            this.height = this.$el.height();
            // on remet à jour les paramètres du zoom de Wpaint 
            this.bgWidth = this.width;
            this.bgHeight = this.height;
            this.bgPosX = 0;
            this.bgPosY = 0;
        ...
        _callShapeFunc: function(e) {
            ...
            //évenement canvas wheel particulier
            if (e.canvasEvent == 'wheel') {
                e.myOffsetX = Math.floor(e.pageX - canvasOffset.left);
                e.myOffsetY = Math.floor(e.pageY - canvasOffset.top);
        ...
        _drawShapeUp: function() {
            ...
            //bernard, on met momentanément notre canvas ici
            ...
            this.canvasTemp2 = cloneCanvas(this.canvas);
            this.$canvasTemp.hide();
        },
        _drawShapeZoom: function(e, factor) {
            var xo = this.canvasTempLeftNew,
                yo = this.canvasTempTopNew;
            // we may need these in other funcs, so we'll just pass them along with the event
            factor = factor || 2;
            e.left = (e.pageX < xo ? e.pageX : xo);
            e.top = (e.pageY < yo ? e.pageY : yo);
            ...
            e.w = e.width - this.options.lineWidth * factor;
            e.h = e.height - this.options.lineWidth * factor;
            //on détermine le centre de gravité où s'applique le zoom
            e.cgX = e.left + e.width / 2;
            e.cgY = e.top + e.height / 2;
            //on récupère les dimensions du canvas de dessin
            e.canvasWidth = this.$canvas.width();
            e.canvasHeight = this.$canvas.height();
            //on détermine le coéficient de zoom minimum
            e.zoomX = e.canvasWidth / e.width;
            e.zoomY = e.canvasHeight / e.height;
            e.zoom = (e.zoomX < e.zoomY ? e.zoomX : e.zoomY);
            // Record the offset between the bg edge and cursor:
            var bgCursorX = e.cgX - this.bgPosX;
            var bgCursorY = e.cgY - this.bgPosY;
            // Use the previous offset to get the percent offset between the bg edge and cursor:
            var bgRatioX = bgCursorX / this.bgWidth;
            var bgRatioY = bgCursorY / this.bgHeight;
            // Update the bg size:
            if (e.zoom > 0) {
                this.bgWidth += this.bgWidth * (e.zoom - 1);
                this.bgHeight += this.bgHeight * (e.zoom - 1);
            }
            else {
                this.bgWidth -= this.bgWidth * (e.zoom - 1);
                this.bgHeight -= this.bgHeight * (e.zoom - 1);
            ...
            this.bgPosX = e.cgX - (this.bgWidth * bgRatioX);
            this.bgPosY = e.cgY - (this.bgHeight * bgRatioY);
            // Prevent zooming out beyond the starting size
            if (this.bgWidth <= e.canvasWidth || this.bgHeight <= e.canvasHeight) {
                this.resize;
            }
            else {
                //on va mettre dans notre canvasTemp, toute l'image déjà dessinée avant de la zoomer
                this.$canvasTemp.hide();
                this.canvasTemp2 = this.canvas;
                this.ctx.translate(this.bgPosX, this.bgPosY)
                this.ctx.scale(e.zoom, e.zoom)
                //            this.ctx.drawImage(this.canvasTemp, this.canvasTempLeftNew, this.canvasTempTopNew);
                this.ctx.drawImage(this.canvasTemp2, 0, 0, e.canvasWidth, e.canvasHeight, 0, 0, e.canvasWidth, e.canvasHeight)

            }


        },
        _drawShapeZoom2: function(e) {
            var zoom = 0.10;
            var deltaY = e.deltaY;
            var offsetX = e.myOffsetX;
            var offsetY = e.myOffsetY;
            //on récupère les dimensions du canvas de dessin
            e.canvasWidth = this.$canvas.width();
            e.canvasHeight = this.$canvas.height();
            // Record the offset between the bg edge and cursor:
            ...
            // Update the bg size:
            if (deltaY < 0) {
                this.bgWidth += this.bgWidth * (zoom);
                this.bgHeight += this.bgHeight * (zoom);
            }
            else {
                this.bgWidth -= this.bgWidth * (zoom);
                this.bgHeight -= this.bgHeight * (zoom);
            ...
            if (this.bgWidth <= e.canvasWidth || this.bgHeight <= e.canvasHeight) {
                ...
                this._updateBgStyle(e);

            }
        },
        _drawTranslateZoom: function(e) {
            e.preventDefault();
            e.canvasWidth = this.$canvas.width();
            e.canvasHeight = this.$canvas.height();            
        ...
        _reset: function(e) {
            this.bgWidth = e.canvasWidth;
            this.bgHeight = e.canvasHeight;
            this.bgPosX = this.bgPosY = 0;
            this._updateBgStyle(e);
        },
        _updateBgStyle: function(e) {
            if (this.bgPosX > 0) {
                this.bgPosX = 0;
            }
            else if (this.bgPosX < e.canvasWidth - this.bgWidth) {
                this.bgPosX = e.canvasWidth - this.bgWidth;
            }
            if (this.bgPosY > 0) {
                this.bgPosY = 0;
            }
            else if (this.bgPosY < e.canvasHeight - this.bgHeight) {
                this.bgPosY = e.canvasHeight - this.bgHeight;
            }
            //on va mettre dans notre canvasTemp, toute l'image déjà dessinée avant de la zoomer
            this.$canvasTemp.hide();
            this._drawCanvas(e)
        ...
        _drawCanvas: function(e) {
            ...
            this.ctx.fillRect(0, 0, e.canvasWidth, e.canvasHeight);
            this.ctx.translate(this.bgPosX, this.bgPosY);
            this.ctx.scale(this.bgWidth / e.canvasWidth, this.bgWidth / e.canvasWidth);
            this.ctx.drawImage(this.canvasTemp2, 0, 0, e.canvasWidth, e.canvasHeight, 0, 0, e.canvasWidth, e.canvasHeight)
            this.ctx.restore();
            //il n'y a pas lieu d'animer si on est en phase d'initialisation
//                        window.requestAnimFrame( function() {
//                        this._drawZoomWheel(e)}
//                        );
        },
    ...
    Menu.prototype = {
        generate: function() {
            ...
            if (this.options.handle) {
                this.$menuHandle = this._createHandle();
            }
            else {
        ...
        _setIndex: function() {
            ...
            if (
            secondaryOffset.top < primaryOffset.top || secondaryOffset.left < primaryOffset.left) {
    ...
    $.fn.wPaint.defaults = {
        path: '/',
        // set absolute path for images and cursors
        theme: 'standard classic',
        // set theme
        autoScaleImage: true,
        // auto scale images to size of canvas (fg and bg)
        autoCenterImage: true,
        // auto center images (fg and bg, default is left/top corner)
        menuHandle: true,
        // setting to false will means menus cannot be dragged around
        menuOrientation: 'horizontal',
        // menu alignment (horizontal,vertical)
        menuOffsetLeft: 5,
        // left offset of primary menu
        menuOffsetTop: 5,
        // top offset of primary menu
        bg: null,
        // set bg on init
        image: null,
        // set image on init
        imageStretch: true,
        // stretch smaller images to full canvans dimensions
        onShapeDown: null,
        // callback for draw down event
        onShapeMove: null,
        // callback for draw move event
        onShapeUp: null,
        // callback for draw up event
        mode: 'rectangle',
        // set mode
        lineWidth: '1',
        // starting line width
